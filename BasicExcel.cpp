...
Worksheet::CellTable::RowBlock::CellBlock::Label::Label() : Record(),
                                                            rowIndex_(0), colIndex_(0), XFRecordIndex_(0)
{
  code_ = CODE::LABEL;
  flag_ = 0;
  wstr_ = NULL;
  dataSize_ = 6; recordSize_ = 4;
}
Worksheet::CellTable::RowBlock::CellBlock::Label::~Label()
{
  Reset();
}
void Worksheet::CellTable::RowBlock::CellBlock::Label::Reset()
{
  if (wstr_) {delete[] wstr_; wstr_ = nullptr;}
}
ULONG Worksheet::CellTable::RowBlock::CellBlock::Label::Read(const char* data)
{
  Reset();
  Record::Read(data);
  LittleEndian::Read(data_, rowIndex_, 0, 2);
  ...
  LittleEndian::Read(data_, XFRecordIndex_, 4, 2);
  short stringSize;
  LittleEndian::Read(data_, stringSize, 6, 2);
  LittleEndian::Read(data_, flag_, 8, 1);
  ...
  if (flag_ == 0) { // compressed UTF16LE string?
    ...
    LittleEndian::ReadString(data_, str, 9, stringSize);
    str[stringSize] = 0;
    mbstowcs(wstr_, str, stringSize);
    wstr_[stringSize] = 0;
    bytesRead += stringSize;
  } else {
    LittleEndian::ReadString(data_, wstr_, 9, stringSize);
    wstr_[stringSize] = 0;
    bytesRead += stringSize*2;
  }
  return RecordSize();
}
ULONG Worksheet::CellTable::RowBlock::CellBlock::Label::Write(char* data)
{
  data_.resize(DataSize());
  LittleEndian::Write(data_, rowIndex_, 0, 2);
  LittleEndian::Write(data_, colIndex_, 2, 2);
...
//			_union.number_ = new Number;
    ...
    case CODE::LABEL:
      //			_union.number_ = new Number;
      bytesRead += _union.label_->Read(data);
      break;
...
ULONG Worksheet::CellTable::RowBlock::CellBlock::Write(char* data)
{
    ...
    case CODE::LABEL:
      bytesWritten += _union.label_->Write(data);
      break;
...
ULONG Worksheet::CellTable::RowBlock::CellBlock::RecordSize()
{
    ...
    case CODE::LABEL:
      return _union.label_->RecordSize();
...
USHORT Worksheet::CellTable::RowBlock::CellBlock::RowIndex()
{
    ...
    case CODE::LABEL:
      return _union.label_->rowIndex_;
...
USHORT Worksheet::CellTable::RowBlock::CellBlock::ColIndex()
{
    ...
    case CODE::LABEL:
      return _union.label_->colIndex_;
	...
	for(int r=0; r<maxRows_; ++r)
	{
		for(int c=0; c<maxCols_; ++c)
		{
			...
			switch(cell->Type())
			{
				...
				{
          if (textQualifier != '\0')
					  os << textQualifier << ::narrow_string(wstring(cell->GetWString())) << textQualifier;
          else
            os << ::narrow_string(wstring(cell->GetWString()));
					break;
				}
			}
			if (c < maxCols_-1)
...
//					cells_[row].resize(col+1);
        ...
        case CODE::LABEL: {
          cells_[row][col].Set(rCellBlocks[j]->_union.label_->wstr_);
